...
## [0.5.1] - 2026-02-12
### Fixed
- 仕入品の「紐づきを外して削除」実行時に `unit_price_history.item_id` の NOT NULL 制約違反で 500 になる問題を修正。`Item.unit_price_history` に `cascade="all, delete-orphan"` を追加し、品目削除時に単価履歴を子レコードとして DELETE するよう変更。
